...
## 📌 Примечания
- Текущая реализация требует передачи `userId` в параметрах запроса для получения привычек. В будущих версиях это будет заменено на автоматическое определение пользователя из JWT токена.
- Для продакшн-среды рекомендуется изменить настройки базы данных (перейти с H2 на PostgreSQL/MySQL) и хранить секретный ключ JWT в защищенном месте.

---
Проект соответствует требованиям MVP трекера привычек с базовой аутентификацией и CRUD-операциями.
